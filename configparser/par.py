    ...
    def _interpolate_some(self, parser, option, accum, rest, section, map, depth):
        ...
        while rest:
            ...
            if c == "%":
                ...
                raise InterpolationSyntaxError(option, section, "'%%' must be followed by '%%' or '(', "
                                                                "found: %r" % (rest,))
...
class ParsingError(Error):
    ...
    def filename(self):
        ...
        warnings.warn("The 'filename' attribute will be removed in future versions.  "
                      "Use 'source' instead.", DeprecationWarning, stacklevel=2)
        return self.source
...
class ConfigParser(MutableMapping):
    ...
    BOOLEAN_STATES = {'1': True, 'yes': True, 'true': True, 'on': True, '0': False, 'no': False, 'false': False,
                      'off': False}

    def __init__(self, defaults=None, dict_type=_default_dict, allow_no_value=False, *, delimiters=('=', ':'),  # 分隔符
                 comment_prefixes=('#', ';'),  # 注释前缀
                 inline_comment_prefixes=None,  # 内联注释前缀
                 strict=True,  # 严格的
                 empty_lines_in_values=True,  # 值中的空行
                 default_section=DEFAULTSECT,  # 默认section
                 interpolation=_UNSET,  # 插值
                 converters=_UNSET  # 转换器
                 ):

        self._dict = dict_type
        self._sections = self._dict()  # 新字典
        self._defaults = self._dict()  # 新字典
        self._converters = ConverterMapping(self)
        self._proxies = self._dict()  # 新字典
    ...
    def readfp(self, fp, filename=None):
        ...
        warnings.warn("This method will be removed in future versions.  "
                      "Use 'parser.read_file()' instead.", DeprecationWarning, stacklevel=2)
    ...
    def getboolean(self, section, option, *, raw=False, vars=None, fallback=_UNSET, **kwargs):
        return self._get_conv(section, option, self._convert_to_boolean, raw=raw, vars=vars, fallback=fallback,
                              **kwargs)
    ...
    def _read(self, fp, fpname):
        ...
        # 逐行读取
        for lineno, line in enumerate(fp, start=1):  # line:内容
            comment_start = sys.maxsize  # comment_start:注释起始值设置的很大，默认没有注释
            # strip inline comments  删除内联注释，猜测相当于一行中跟在代码后的注释
            inline_prefixes = {p: -1 for p in
                               self._inline_comment_prefixes}  # self._inline_comment_prefixes:空元组 -> inline_prefixes:空字典
            #                                     直接跳过(因为↓↓↓为空)
            ...
            # strip full line comments  删除整行注释，这个很好理解，就像本行注释一样
            for prefix in self._comment_prefixes:  # self._comment_prefixes:('#', ';')
                if line.strip().startswith(prefix):
                    comment_start = 0
                    break
            if comment_start == sys.maxsize:  # 仍然很大，没有注释
                comment_start = None
            value = line[:comment_start].strip()  # 把有效的键值（或节的名称）对截了出来
            if not value:  # 是空行
                if self._empty_lines_in_values:  # self._empty_lines_in_values默认为True
                    # add empty line to the value, but only if there was no comment on the line
                    # 在值中添加空行，但前提是该行没有注释
                    if (comment_start is None and cursect is not None and  # 默认为None哦
                            optname and  # 默认为None哦
                            cursect[optname] is not None):
                        cursect[optname].append('')  # newlines added at join
                else:
                    # empty line marks end of value  空行标记值的结束
            ...
            # a section header or option header?  区段标题
            else:
        ...
        # if any parsing errors occurred, raise an exception  如果出现任何分析错误，则抛出异常
        if e:
            raise e
    ...
    def __read_defaults(self, defaults):
