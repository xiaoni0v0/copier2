...
int conf()
{
    ...
    rff<int> R_CopyFlush = ip.get_v_int("copier", "CopyFlush", 1024), R_MaxSendSize = ip.get_v_int("net_work",
                                                                                                   "MaxSendSize", -1);
    rff<bool> R_LogMode = ip.get_v_bool("copier", "LogMode", true), R_SendPPT = ip.get_v_bool("net_work", "SendPPT",
                                                                                              false);
    rff<string> R_PowerPointPath = ip.get_v_string("copier", "PowerPointPath"), R_CopyPath = ip.get_v_string("copier",
                                                                                                             "CopyPath"), R_IgnoreDisk = ip.get_v_string(
            "copier", "IgnoreDisk"), R_ServerAddr = ip.get_v_string("net_work",
                                                                    "ServerAddr"), R_SecretKey = ip.get_v_string(
            "net_work", "SecretKey");
    // init
    ...
    // return
    return ret_read | R_LogMode.status() | R_PowerPointPath.status() | R_CopyPath.status() | R_IgnoreDisk.status() |
...
void log(const char *format, ...)
{
    if (C_LogMode)
    {
        tm *p = get_tm();
        // 输出时间
        printf("[%04d/%02d/%02d %02d:%02d:%02d] ", p->tm_year + 1900, p->tm_mon + 1, p->tm_mday, p->tm_hour, p->tm_min,
               p->tm_sec);
        // 输出调试信息
...
HANDLE executePPTFile()
{
    return ShellExecuteA(nullptr, "open", ("\"" + C_PowerPointPath + "\"").c_str(), ("\"" + f_path + "\"").c_str(),
...
void checkEnvironment_with1arg()
{
    ...
    if (getFileSize(C_PowerPointPath) != -1)
    {
        ...
        MessageBoxW(nullptr, L"环境正常！可以使用！", copyRight, MB_ICONINFORMATION);
    }
    else
    {
        log("未找到 PowerPoint 程序！请检查配置！\n");
        MessageBoxW(nullptr, L"未找到PowerPoint程序！\n请检查配置！", copyRight, MB_ICONERROR);
    }
...
void testNetStatus_with1arg()
{
    ...
    if (s.sta)
    {
        MessageBoxW(nullptr, L"网络连接失败！", copyRight, MB_ICONERROR);
        return;
    }
    log("version: %d\n", s.send_version());
    log("what: %d\n", s.send_what(0));
    ...
    {
        case 0:
            MessageBoxW(nullptr, L"连接网络成功！密码正确！", copyRight, MB_ICONINFORMATION);
            break;
        case 1:
            MessageBoxW(nullptr, L"密码错误！", copyRight, MB_ICONERROR);
            break;
        case -1:
            MessageBoxW(nullptr, L"网络连接失败！", copyRight, MB_ICONERROR);
            break;
...
void sendFileAfterCopy_with2args()
{
    ...
    {
        if (MessageBoxW(nullptr, L"是否发送该文件？", copyRight, MB_YESNO) == IDNO)
        {
            return;
        }
    }
...
int main(int _argc, char *_argv[])
{
    ...
    [net_work]
        ...
        C_CopyFlush > 0 ? "" : " (不是合法数值)", C_ServerAddr.c_str(), C_SendPPT, C_SecretKey.c_str(), C_MaxSendSize,
        C_MaxSendSize >= -1 ? "" : " (不是合法数值)");
